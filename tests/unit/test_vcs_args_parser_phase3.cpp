...
public:
    static std::string createTempFile(const std::string& content, const std::string& suffix = ".sv") {
        ...
        std::string filename = "/tmp/test_phase3_" + std::to_string(counter++) + suffix;
...
TEST_CASE("VCS Args Parser - Phase 3: SystemVerilog Assertions", "[vcs][phase3][assert]") {
    ...
    SECTION("-assert hier=<filename>") {
        std::string hierFile = TempFileHelper::createTempFile(
            "assert_hierarchy_config\n"
            "module top.*\n"
            "  enable all\n", ".hier");
        ...
        TempFileHelper::cleanup(hierFile);
    }
    ...
    SECTION("Multiple assertion options") {
        std::string hierFile = TempFileHelper::createTempFile("assert config\n", ".hier");
        std::string testFile = TempFileHelper::getTestDataFile("core.sv");
        std::string hierArg = "hier=" + hierFile;
...
TEST_CASE("VCS Args Parser - Phase 3: Code Coverage", "[vcs][phase3][coverage]") {
    ...
    SECTION("-cm_exclude coverage exclude") {
        std::string excludeFile = TempFileHelper::createTempFile(
            "# Exclude patterns\n"
            "*.testbench.*\n"
            "*.debug_*\n", ".exclude");
        ...
        TempFileHelper::cleanup(excludeFile);
    }
    ...
    SECTION("Complete coverage configuration") {
        std::string hierFile = TempFileHelper::createTempFile("hier config\n", ".hier");
        std::string excludeFile = TempFileHelper::createTempFile("exclude config\n", ".exclude");
        std::string testFile = TempFileHelper::getTestDataFile("core.sv");
...
TEST_CASE("VCS Args Parser - Phase 3: Complex Integration", "[vcs][phase3][integration]") {
    ...
    SECTION("Complete Phase 1+2+3 example") {
        ...
        TempFileHelper::cleanup(assertHier);
        TempFileHelper::cleanup(coverageHier);
        TempFileHelper::cleanup(coverageExclude);
    }
}