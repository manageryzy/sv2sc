...
public:
    static std::string createTempFile(const std::string& content, const std::string& suffix = ".sv") {
        ...
        std::string filename = "/tmp/test_phase4_" + std::to_string(counter++) + suffix;
...
TEST_CASE("VCS Args Parser - Phase 4: Power Analysis", "[vcs][phase4][power]") {
    ...
    SECTION("-upf UPF file specification") {
        std::string upfFile = TempFileHelper::createTempFile(
            "# UPF Power Configuration\n"
        ...
        TempFileHelper::cleanup(upfFile);
    }
    ...
    SECTION("Combined power analysis options") {
        std::string upfFile = TempFileHelper::createTempFile("UPF content\n", ".upf");
        std::string testFile = TempFileHelper::getTestDataFile("core.sv");
...
TEST_CASE("VCS Args Parser - Phase 4: Advanced Optimization", "[vcs][phase4][optimization]") {
    ...
    SECTION("+optconfigfile+ optimization config") {
        std::string configFile = TempFileHelper::createTempFile(
        ...
        TempFileHelper::cleanup(configFile);
    }
...
TEST_CASE("VCS Args Parser - Phase 4: Verification Methodology", "[vcs][phase4][verification]") {
    ...
    SECTION("Verification methodologies") {
        std::string ovaFile = TempFileHelper::createTempFile(
            "# OVA Configuration\n"
            "assertion_library = standard\n", ".ova");
        ...
        TempFileHelper::cleanup(ovaFile);
    }
}
